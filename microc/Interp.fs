...
let rec allocate (typ, x) (env0, nextloc) sto0 : locEnv * store = 
    ...
    let (nextloc1, v, sto1) =
        ...
        // 默认值是 -1
        | _ -> (nextloc, -1, sto0)
    (bindVar x v (env1, nextloc1) sto1)
...
let initEnvAndStore (topdecs : topdec list) : locEnv * funEnv * store = 
    ...
    let rec addv decs locEnv funEnv store = 
        ...
        | Fundec (_, f, xs, body) :: decr ->
          ...
          (addv decr locEnv ((f, (xs, body)) :: funEnv1) store)
...
and eval e locEnv gloEnv store : int * store = 
    ...
    | Assign(acc,ope,e) -> let (loc, store1) = access acc locEnv gloEnv store
                           let i1 = getSto (store1) loc
                           let (i2,  store2) = eval e locEnv gloEnv store1
                           let res =
                               match ope with
                               | "="  -> i2
                               | "+=" -> i1 + i2
