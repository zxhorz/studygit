...
let rec check env x = 
    match env with 
    | []         -> env
    | (y, v)::yr -> if x=y then failwith (x + " has existed") else check yr x
...
let allocate (kind : int -> var) (typ, x) (varEnv : varEnv) : varEnv * instr list =
    ...
    let env1 = (env) @ (check env x)
    match typ with
    | TypA (TypA _, _) -> 
      raise (Failure "allocate: array of arrays not permitted")
    | TypA (t, Some i) -> 
      let newEnv = ((x, (kind (fdepth+i), typ)) :: env1, fdepth+i+1)
      let code = [INCSP i; GETSP; CSTI (i-1); SUB]
      (newEnv, code)
    | _ -> 
      let newEnv = ((x, (kind (fdepth), typ)) :: env1, fdepth+1)
      let code = [INCSP 1]
      (newEnv, code)
    ...
    let rec addv decs varEnv funEnv = 
        ...
        | dec::decr  -> 
          ...
          | Fundec (tyOpt, f, xs, body) ->
            let funEnv1 =(funEnv)@(check funEnv f)
            addv decr varEnv ((f, (newLabel(), tyOpt, xs)) :: funEnv1)
    addv topdecs ([], 0) []
    ...
    | Assign(acc, ope, e) -> 
             ...
             @ (match ope with
                | "="  -> cExpr e varEnv funEnv 
                          @ [STI]
                | "+=" -> (cAccess acc varEnv funEnv) @ [LDI] @ cExpr e varEnv funEnv @ [ADD;STI]
                | "-=" -> (cAccess acc varEnv funEnv) @ [LDI] @ cExpr e varEnv funEnv @ [SUB;STI]
                | "*=" -> (cAccess acc varEnv funEnv) @ [LDI] @ cExpr e varEnv funEnv @ [MUL;STI]
                | "/=" -> (cAccess acc varEnv funEnv) @ [LDI] @ cExpr e varEnv funEnv @ [DIV;STI]
